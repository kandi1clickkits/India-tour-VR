...
SET REPO_DOWNLOAD_URL=https://github.com/kandi1clickkits/India-tour-VR/releases/download/v1.0.0/India-tour-VR.zip
SET REPO_NAME=India-tour-VR.zip
SET EXTRACTED_REPO_DIR=India-tour-VR
SET FILE_NAME=-m http.server 8000
where /q python
IF ERRORLEVEL 1 (
	ECHO==========================================================================
...
where /q python
IF ERRORLEVEL 1 (
	%PY_LOCATION%\python.exe %FILE_NAME%"
)	ELSE (
python %FILE_NAME%"
)
)
